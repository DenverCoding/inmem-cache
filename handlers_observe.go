...
- items carry a cache-owned 'ts' field (signed int64, microseconds since unix epoch — time.Now().UnixMicro()). ts is set by the cache on every write that touches the item (/append, /upsert create+replace, /update, /counter_add create+increment, /warm, /rebuild, /inbox). Clients MUST NOT supply ts on writes — every write rejects a non-zero client-supplied ts with 400. ts is observability only: not searchable, not indexed, not used for ordering. It captures "when did the cache last write this item" — useful for logging and last-activity stamping on counters. Microsecond granularity (not milliseconds) keeps two writes inside the same millisecond distinguishable.
...
- /head and /tail responses carry a "truncated" boolean: true when more matching items exist beyond the returned window
- /head, /tail and /get responses carry "count" and "approx_response_mb" fields alongside duration_us so the read family produces a uniform response shape; on /head and /tail approx_response_mb also lets clients see how close they sit to the per-response cap (see below) without having to measure the body themselves
- id is optional
...
POST /multi_call - sequentially dispatch N independent sub-calls in one HTTP roundtrip; body is {"calls": [{"path": "/get|/append|...", "query": {...}, "body": {...}}, ...]}; allowed paths: /append, /get, /head, /tail, /update, /upsert, /counter_add, /delete, /delete_up_to; response is {ok, count, results: [{status, body}, ...], approx_response_mb, duration_us} in input order. No cross-call atomicity — a write at index 0 stays applied even if index 1 fails. Outer envelope honours the per-response cap (SCOPECACHE_MAX_RESPONSE_MB); slot bodies that would push the envelope past the cap are replaced with a minimal {"ok":true|false,"response_truncated":true} marker while the slot's status is preserved.

ADMIN-ONLY (gated outside the cache by socket permissions or Caddyfile route):
POST /admin - operator-elevated dispatcher; same {"calls":[...]} body and {"results":[...]} envelope as /multi_call. Reaches reserved scopes (_*) directly; no rewrite. Wider whitelist than /multi_call: /append, /get, /head, /tail, /update, /upsert, /counter_add, /delete, /delete_up_to, /delete_scope, /delete_guarded, /warm, /rebuild, /wipe, /stats, /delete_scope_candidates. Excluded: /help (text/plain), /render (raw bytes don't fit a JSON results array), /multi_call/guarded/inbox/admin (self-reference loops). /warm, /rebuild, /wipe, /delete_scope, /delete_guarded, /stats, /delete_scope_candidates are reachable ONLY through /admin — they are not on the public mux. /stats and /delete_scope_candidates are admin-only because they enumerate every scope name in the store, which leaks reserved scopes (_tokens, _guarded:*, _counters_*) and per-scope heat metadata in multi-tenant deployments. Registered only when EnableAdmin is set: standalone defaults true (Unix-socket permission gating); Caddy module defaults false (operator must opt in via 'enable_admin yes' AND add a Caddyfile route guard, since /admin has no body-level auth).

OPTIONAL ENDPOINTS (registered only when configured):
POST /guarded - tenant-facing multi-call gateway; body {"token":"<opaque>","calls":[...]} derives capability_id = HMAC_SHA256(SCOPECACHE_SERVER_SECRET, token), gates on _tokens membership, and rewrites every sub-call's scope to _guarded:<capability_id>:<original-scope>. Whitelist (closed): /append, /get, /head, /tail, /update, /upsert, /counter_add, /delete, /delete_up_to. Excludes /delete_scope, /stats, /delete_scope_candidates (cross-tenant visibility / namespace ownership), /wipe, /warm, /rebuild, /delete_guarded (admin-only), and /render (raw bytes don't fit the results-array envelope). Registered only when SCOPECACHE_SERVER_SECRET is set; otherwise the route returns 404.
POST /inbox - shared write-only ingestion; single /append per request (no envelope). Cache assigns id (capability_id:<32-hex random>, i.e. 16 random bytes hex-encoded) and ts (now in microseconds). Tenants cannot read what they wrote — reads happen via /admin. Registered only when SCOPECACHE_SERVER_SECRET is set AND at least one inbox scope name is configured (SCOPECACHE_INBOX_SCOPES on the standalone binary; repeated 'inbox_scope <name>' directives in the Caddy module).
